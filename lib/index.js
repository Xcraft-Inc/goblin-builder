...
exports.loadAppConfig = (appId, appDir, configJson = {}) => {
  ...
  if (hordesCfg && hordesCfg.hordes) {
    hordesCfg.hordes
      ...
      .forEach(appId => exports.loadAppConfig(appId));
  }
...
exports.extractAllDeps = (libDir, configJson) => {
  const deps = exports.extractConfigDeps(libDir, configJson);

  const filters = [/^xcraft-(core|contrib)/];
  const serverCfg = configJson['xcraft-core-server'];
  if (serverCfg) {
  ...
  const extract = () => {
    const _deps = Object.keys(deps)
      .map(
        dep =>
          JSON.parse(fse.readFileSync(path.join(libDir, dep, 'package.json')))
            .dependencies
      )
      .filter(dep => !deps[dep] && filters.some(filter => filter.test(dep)));
