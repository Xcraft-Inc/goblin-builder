...
class Builder {
  ...
  *_assets(isDev, next) {
    ...
    deps.forEach((dep) => {
      ...
      const output = path.join(this._buildDir, 'lib', dep);
    ...
    this._packageJson.dependencies = Object.assign(
      ...deps.map((dep) => ({[path.basename(dep)]: `file:${dep}`})),
      this._packageJson.dependencies
    );
  ...
  _cleanup() {
    process.noAsar = true;
    fse.removeSync(path.join(this._buildDir, 'lib'));
    ...
    for (const renderer in config.renderers) {
      for (const filename in config.renderers[renderer]) {
        const componentPath = path.join(
          this._releaseDir,
          'node_modules',
          config.renderers[renderer][filename].entry
        );
        const mainGoblin = config.renderers[renderer][filename].goblin;
        const assets = config.renderers[renderer][filename].assets;
