...
class Builder {
  constructor(quest, options) {
    ...
    if (options.appDir) {
      this._configJson = utils.loadAppConfig(this._app.appId, options.appDir);
    }
     ...
     */
    if (
      this._configJson[this._app.name] &&
      this._configJson[this._app.name]['goblin-client']
    ) {
      this._mainGoblinModule = this._configJson[this._app.name][
        'goblin-client'
      ].mainGoblinModule;
    }
    ...
    const deps = utils.extractConfigDeps(options.libDir, this._configJson);
    ...
    this._scrcDev = {
      ...
      exclude: ['xcraft-dev-rules', 'babel-env', 'generic-js-env', 'mai-chai'],
    };
