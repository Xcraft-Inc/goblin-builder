...
class Builder {
  ...
  *_extraBuilds() {
  ...
  extractStartCraft(libDir, dep, isDev, deps = {}, _recurse = false) {
    let pkgJson = path.join(libDir, dep, 'package.json');
    let def;

    try {
      def = fse.readJSONSync(pkgJson);
    } catch (ex) {
      if (_recurse) {
        throw ex;
      }
      /* Only the first (top level) module can exists only in node_modules/
       * instead of lib/.
       */
      const nodeModulesDir = path.join(path.dirname(libDir), 'node_modules');
      pkgJson = path.join(nodeModulesDir, dep, 'package.json');
      def = fse.readJSONSync(pkgJson);
    }
    ...
    const extract = (defDeps) => {
      Object.keys(defDeps).forEach((dep) => {
        if (!deps[dep] && fse.existsSync(path.join(libDir, dep))) {
          ...
          this.extractStartCraft(libDir, dep, isDev, deps, true);
        }
      });
    };
