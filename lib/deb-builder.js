...
class DebBuilder extends Builder {
  ...
  *_clean(next) {
    yield fse.remove(this._buildDir, next);
    yield fse.mkdirs(this._buildDir, next);
  }
  ...
  *_assets(next) {
    ...
    [
      ...
      const output = path.join(this._buildDir, item.name);
      fse.writeFile(
        output,
    ...
    fse.writeFile(
      path.join(this._buildDir, 'default_variables'),
    ...
    fse.writeFile(
      path.join(this._buildDir, 'config.js'),
