...
class AppBuilder extends Builder {
  ...
  *_esign(configuration, next) {
    const whereIs = require('xcraft-core-utils/lib/whereIs.js');
    const signtool = process.env.SIGNTOOL_PATH ?? whereIs('esign.exe');
    if (!signtool) {
      return;
    }

    const file = configuration.path;
    this._quest.log.info(`spawn ${signtool} -l ${file}`);
    yield this._xProcess.spawn(signtool, ['-l', file], {}, next);
  ...
  *_electron(next) {
    ...
    */
    ...
    const options = {
      ...
      config: {
        ...
        win: {
          ...
          sign: this._release
            ? async (configuration) => await this._esign(configuration)
            : null,
        },
        linux: {
          target: ['AppImage', 'deb'],
    ...
    switch (process.platform) {
      case 'darwin':
        options.config.asarUnpack.push(
          '**/node_modules/**/*.dylib',
          '**/node_modules/**/*.so'
        );
        break;
