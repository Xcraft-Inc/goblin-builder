...
class AppBuilder extends Builder {
  ...
  *_webpack() {
  ...
  _esignPath() {
    ...
    this._esign = process.env.SIGNTOOL_PATH ?? whereIs('esign.exe');
  }
  ...
  *_esign(configuration, next) {
    ...
    this._quest.log.info(`spawn ${this._esign} -l ${file}`);
    yield this._xProcess.spawn(this._esign, ['-l', file], {}, next);
